            ...
            while not self._voice_stop.is_set():
                try:
                    ...
                    base_instruction = (
                        ...
                        "2. SIMPLE ACTIONS: Use native tools (system_control, todo_control, memory_control, weather_control, control_servo, map_control, node_control, web_search, clipboard_control, sensor_control, media_generation) directly to fulfill simple requests like changing volume, rotating the servo motor, showing visual cards/widgets, getting location info, checking weather, or managing todo/memories. Do NOT call call_agent for these.\n\n"
                        "3. DELEGATED COMPLEX TASKS & SCREEN VISION: If the request is complex (writing/editing files, running terminal commands, coding), requires tools that are not in your native list (like file_control, browser_control, input_control, screen_control), or if you think your native tools are not capable of satisfying the request, you MUST immediately call the call_agent tool. Do NOT try to guess paths or write terminal commands yourself to bypass missing tools. Instead, delegate the task by calling call_agent(prompt='...'). For screen inspection, always immediately call call_agent(prompt='Take a screenshot of the current screen and analyze it to answer the user').\n\n"
                        "4. ALERTS / REMINDERS: If you receive a [SYSTEM_ALERT] containing a user reminder, announce it dynamically to the user in a natural, caring, and playful Hinglish tone.\n\n"
