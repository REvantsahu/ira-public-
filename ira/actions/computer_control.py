...
def computer_control(
    ...
    # Keyboard-interactive actions need the target window to have focus.
    # The HUD overlay can steal focus, so we hide it before these actions
    # and restore it after. Mouse actions (click, drag, etc.) already get
    # this via pyautogui monkeypatch in hud_overlay.py.
    _KEYBOARD_ACTIONS = {"type", "smart_type", "press", "hotkey", "clear_field", "paste"}
    hud_hidden = False
    ...
    try:
        ...
        # Restore HUD overlay after keyboard actions
        if hud_hidden:
            try:
                import screen as _scr
